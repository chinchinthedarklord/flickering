...
def weibull(x, k, lmbda):
	result = 1 - np.exp(- (abs((x + lmbda * gamma(1 + 1/k)) / lmbda)) **k)
	return result
	
def function(x, k, lmbda):
	result = ((x + lmbda * gamma(1 + 1/k)) / lmbda) **k
	return result
...
weibull_model = Model(weibull)
result = weibull_model.fit(probability, x = flickering_sorted, k = k, lmbda = lmbda)
